    ...
    <header className="bg-[#5ACCC3] text-white shadow-lg sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="font-bold text-2xl tracking-wider">
          <Link to="/lab/dashboard" className="text-white no-underline hover:text-opacity-90 transition-all flex items-center">
        ...
        <nav className="flex space-x-8">
          {[
            ...
            { name: "PATIENTS", path: "/lab/patients" },
            { name: "RESULTS", path: "/lab/results" },
            { name: "REPORTS", path: "/lab/reports" },
            { name: "MESSAGES", path: "/lab/messages" }
          ].map((item) => (
            <Link
              key={item.path}
              to={item.path}
