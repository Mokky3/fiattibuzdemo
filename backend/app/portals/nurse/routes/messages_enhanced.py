"""Nurse portal – enhanced messaging router
...
router = APIRouter(prefix="/messages", tags=["Nurse · Messages"])
...
def normalize_clinic_id(db: Session, current_user: AuthenticatedUser, clinic_id: str) -> str:
    ...
    if clinic_id == "default" or not clinic_id:
        ...
        if user and user.organization_id:
            ...
            logger.warning(f"Nurse {current_user.user_id} has no organization_id, using 'default'")
    return clinic_id

from app.portals.nurse.schemas.messages_enhanced import (
    SendMessageRequest,
    ConversationSummary,
    MessageThread,
    MessageStats,
    ...
    try:
        ...
        if not rbac_service.can_access_clinic(current_user, effective_clinic_id):
            problem = create_problem_detail(
                ...
                status=403,
                detail="Nurse does not have access to this clinic",
            ...
            if patient and patient.user_id:
                ...
                if not rbac_service.can_access_patient(current_user, payload.recipient_id, effective_clinic_id):
                    problem = create_problem_detail(
                        ...
                        status=403,
                        detail="Nurse does not have access to this patient",
                        trace_id=get_trace_id()
                    )
                    raise HTTPException(status_code=403, detail=problem.dict())
        ...
        # Get nurse's full name from User model
        user_obj = db.query(User).filter(User.id == current_user.user_id).first()
        nurse_name = f"{user_obj.first_name} {user_obj.last_name}".strip() if user_obj and user_obj.first_name and user_obj.last_name else current_user.email
                ...
                try:
                    ...
                    if not file_url:
                        ...
                        if doc_id:
                            ...
                            if doc_type == "report":
                                file_url = f"/api/v1/nurse/reports/{doc_id}"
                            elif doc_type == "lab_result":
                                file_url = f"/api/v1/nurse/lab-results/{doc_id}"
                            elif doc_type == "imaging":
                                file_url = f"/api/v1/nurse/imaging/{doc_id}"
                            elif doc_type == "prescription":
                                file_url = f"/api/v1/nurse/prescriptions/{doc_id}"
                            elif doc_type == "clinical_note" or doc_type == "report":
                                # Clinical notes are often reports
                                file_url = f"/api/v1/nurse/reports/{doc_id}"
                            else:
                                # Generic document URL
                                file_url = f"/api/v1/nurse/documents/{doc_id}"
                        else:
...
async def upload_attachment(
    ...
    try:
        ...
        if not rbac_service.can_access_patient(current_user, recipient_id, clinic_id):
            problem = create_problem_detail(
                error_type=ErrorType.AUTHORIZATION_ERROR,
                title="Patient Access Denied",
                status=403,
    ...
    try:
        ...
        for conv in conversations_result["conversations"]:
            conversations.append(ConversationSummary(
                ...
                fhir_conversation_id=conv.get("fhir_conversation_id")
            ))
...
async def get_conversation_messages(
    ...
    try:
        ...
        if is_patient:
            ...
            if not thread:
                thread_id = f"thread_{recipient_id}_{current_user.user_id}"
                ...
                # Add participants: nurse and patient
                if patient and patient.user_id:
                    # Add nurse as participant
            ...
            # Look for threads where both users are participants
            ...
            threads = db.query(MessageThreadModel).join(MessageThreadParticipant).filter(
                MessageThreadParticipant.user_id.in_([current_user.user_id, recipient_user_id]),
            ...
            for t in threads:
                ...
                if {current_user.user_id, recipient_user_id}.issubset(participant_user_ids):
                    thread = t
                    break
...
async def get_unread_messages(
    ...
    """Get all unread messages for the nurse."""
    try:
        # Normalize clinic_id if it's "default"
        effective_clinic_id = normalize_clinic_id(db, current_user, clinic_id)
...
async def get_message_stats(
    ...
    """Get message statistics for the nurse."""
    try:
        # Normalize clinic_id if it's "default"
        effective_clinic_id = normalize_clinic_id(db, current_user, clinic_id)
...
async def get_patients_for_messaging(
    ...
    try:
        ...
        patients = await messaging_service.get_patients_for_messaging(
            doctor_id=str(current_user.user_id),  # Reusing doctor_id parameter for nurse
            clinic_id=effective_clinic_id
        )
...
async def get_patient_documents(
    ...
    try:
        ...
        # 1. Get DocumentReference directly from database (reports) - if table exists
        try:
        ...
        # 2. Get Imaging Results (from DocumentReference)
        try:
        ...
        # 4. Get Other Document References
        try:
