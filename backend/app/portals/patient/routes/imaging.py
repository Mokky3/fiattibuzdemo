"""Patient portal - Medical Imaging (OHIF/DICOM) router
Provides access to OHIF viewer for viewing patient's own DICOM images
"""
...
router = APIRouter(prefix="/imaging", tags=["Patient · Medical Imaging"])
...
async def fetch_patient_studies_from_orthanc(patient_id: str) -> List[Dict]:
    """Fetch studies from Orthanc for a specific patient"""
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            # Fetch details for each study and filter by patient ID
            studies = []
            for study_id in study_ids:
                try:
                    ...
                    if study_response.status_code == 200:
                        ...
                        # Only include studies for this patient
        ...
        logger.error(f"Error fetching patient studies from Orthanc: {e}")
        return []
...
def get_patient_id_from_db(patient_user: PatientUser, db: Session) -> Optional[str]:
    """Get patient ID from database to match with Orthanc PatientID"""
    try:
        # Get patient record
        patient = db.query(PatientORM).filter(PatientORM.user_id == patient_user.id).first()
        if not patient:
            return None
        
        # Try to get patient ID from various sources
        # First, try patient_code
        if patient.patient_code:
            return patient.patient_code
        
        # Try MRN from organization_patients
        ...
        if org_patient and org_patient.local_mrn:
            return org_patient.local_mrn
        
        # Fallback to patient_id as string
        return str(patient.patient_id)
    except Exception as e:
        logger.error(f"Error getting patient ID from DB: {e}")
        return None
...
async def list_my_studies(
    ...
    """List DICOM studies for the current patient"""
    try:
        # Get patient ID to match with Orthanc
        patient_id = get_patient_id_from_db(current_patient, db)
        if not patient_id:
            # Return empty list if patient ID not found
            return SuccessResponse(data=StudiesListResponse(
                studies=[],
                total=0
            ))
        
        studies_data = await fetch_patient_studies_from_orthanc(patient_id)
        ...
        logger.error(f"Error listing patient studies: {e}")
        raise HTTPException(
...
async def get_viewer_url(
    ...
    """Get OHIF viewer URL for a specific study or show patient's study list"""
    try:
        # Get patient ID for validation
        patient_id = get_patient_id_from_db(current_patient, db)
        
        # If study_instance_uid is provided, validate it belongs to patient
        if study_instance_uid:
            # Verify the study belongs to this patient
            studies_data = await fetch_patient_studies_from_orthanc(patient_id or "")
            study_found = False
            for item in studies_data:
                study_data_obj = item["data"]
                if study_data_obj.get("MainDicomTags", {}).get("StudyInstanceUID") == study_instance_uid:
                    study_found = True
                    break
            
            if not study_found and patient_id:
                raise HTTPException(
        ...
        # If study_id is provided, fetch StudyInstanceUID from Orthanc and validate
        if study_id:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    study_patient_id = study_data.get("MainDicomTags", {}).get("PatientID", "")
                    if patient_id and study_patient_id != patient_id:
                        raise HTTPException(
                            status_code=status.HTTP_403_FORBIDDEN,
                            detail="You do not have access to this study"
                        )
        ...
        # If no study specified, return viewer URL for patient's study list
        # Filter by patient ID in OHIF if possible, otherwise show all (OHIF will filter client-side)
...
async def get_study_details(
    ...
    """Get detailed information about a specific study (patient's own studies only)"""
    try:
        # Get patient ID for validation
        patient_id = get_patient_id_from_db(current_patient, db)
